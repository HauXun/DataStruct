...
float MaxDTB(LIST list)
{
	float max = list.pHead->data.dtb;
...
string TenSinhVien(string hoTen)
{
	int length = hoTen.length();
	int i;
...
void MaxDTB_WriteFile(LIST list)
{
	float max = MaxDTB(list);
	ofstream out;
	out.open("MAXDTB.txt", ios::out);
	for (NODE* i = list.pHead; i != NULL; i = i->pNext)
		if (i->data.dtb == max)
		{
			Write(out, i->data);
...
void TimSinhVien_Ten(LIST list, string tenSV)
{
	for (NODE* i = list.pHead; i != NULL; i = i->pNext)
	{
		string ten = TenSinhVien(i->data.hoTen);
